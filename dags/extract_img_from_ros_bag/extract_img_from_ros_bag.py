...
def bag_filename_syntax_valid(bag_path):
    ...
    if files:
        for bag in files:
            ...
            object_name = splited_filename[0]
            location_name = splited_filename[1]
            filename_date = splited_filename[2]

            if not isinstance(object_name, str) or not isinstance(location_name, str):
                logging.info(

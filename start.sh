...
start_dev() {
    e_airflow_container_starting
    AIRFLOW_DAG_DIR=${AIRFLOW_DAG_DIR} HOST_ROOT_FOLDER=${CURRENT_DIR}  docker-compose -f ${DOCKER_DIR}/docker-compose.yml -f ${DOCKER_DIR}/docker-compose-local.yml --env-file ./.env up -d --build || e_error "Error while starting '${AIRFLOW_DOCKER_IMAGE_NAME}'"
}

...
if [[ ${BUILD_ENV} == 'prod' ]]; then
    ...
    echo ${CURRENT_DIR}
    AIRFLOW_DAG_DIR=${AIRFLOW_DAG_DIR} HOST_ROOT_FOLDER=${CURRENT_DIR}  docker-compose -f ${DOCKER_DIR}/docker-compose.yml -f ${DOCKER_DIR}/docker-compose-prod.yml up -d|| error "Error while starting '${AIRFLOW_DOCKER_IMAGE_NAME}'"
else
    echo "#########################################################################"
    echo
    echo "Launching sonia-auv airflow docker containers"
